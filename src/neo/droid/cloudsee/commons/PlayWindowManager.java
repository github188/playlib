...
public class PlayWindowManager implements View.OnClickListener,
	...
	public boolean chargeByIndex(int index) {
		boolean result = false;

		int size = mWindowList.size();
		if (size > index && index >= 0) {
			mWindowList.get(index).charge();
			result = true;
		}
	...
	public boolean chargeAll() {
		boolean result = true;

		int size = mWindowList.size();
		for (int i = 0; i < size; i++) {
			if (false == mWindowList.get(i).charge()) {
				result = false;
			}
		}
			...
			for (int i = 0; i < lines; i++) {
				...
				for (int j = 0; j < cols; j++) {
					...
					if (null != player) {
						...
						if (channel.getIndex() < 0) {
							TextView text = (TextView) border
									.findViewById(ID_INFO_TEXT);
	...
	private class PlayWindow {
		...
		public boolean charge() {
			...
			if (null != mChannel && null != mChannel.getSurfaceView()) {
				mChannel.getSurfaceView().setVisibility(View.GONE);
				index = mChannel.getIndex();
				mChannel.setSurfaceView(genSurfaceView());
				result = true;
			}
